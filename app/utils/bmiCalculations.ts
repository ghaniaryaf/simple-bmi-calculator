...
export const calculateStandardBMI = (
  weightKg: number,
  heightM: number,
): number => {
...
export const calculateWHOAdjustedBMI = (
  bmi: number,
  ageNum: number,
  gender: Gender,
): number => {
...
export const calculateNewFormulaBMI = (
  weightKg: number,
  heightM: number,
  ageNum: number,
  gender: Gender,
...
export const classifyBMI = (
  bmi: number,
  ageNum: number,
): { category: string; color: string; healthRisk: string } => {
  let category = "";
  let color = "";
  let healthRisk = "";
  ...
  if (ageNum < 2) {
    if (bmi < 14) {
      ...
      color = "#10b981";
      healthRisk = "Healthy growth";
    } else if (bmi < 20) {
      category = "At risk of overweight";
      color = "#f59e0b";
      healthRisk = "Monitor growth";
    } else {
      category = "Overweight";
      color = "#ef4444";
      healthRisk = "High weight for age";
    }
  } else if (ageNum >= 2 && ageNum <= 19) {
    if (bmi < 5) {
      category = "Underweight";
      color = "#3b82f6";
      healthRisk = "Below 5th percentile";
    } else if (bmi < 85) {
      category = "Healthy weight";
      color = "#10b981";
      healthRisk = "5th-84th percentile";
    } else if (bmi < 95) {
      category = "Overweight";
      color = "#f59e0b";
      healthRisk = "85th-94th percentile";
    } else {
      category = "Obese";
      color = "#ef4444";
      healthRisk = "95th+ percentile";
    }
  } else {
    if (bmi < 18.5) {
      category = "Underweight";
      color = "#3b82f6";
      healthRisk = "Nutritional deficiency";
    } else if (bmi < 25) {
      category = "Normal weight";
      color = "#10b981";
      healthRisk = "Low risk";
  ...
  if (ageNum >= 65) {
    if (bmi < 23) {
      category = "Underweight";
      color = "#3b82f6";
      healthRisk = "Increased mortality risk";
    } else if (bmi < 30) {
      category = "Healthy weight";
      color = "#10b981";
      healthRisk = "Optimal for elderly";
    } else {
      category = "Overweight";
      color = "#f59e0b";
      healthRisk = "Monitor health";
    }
  }
